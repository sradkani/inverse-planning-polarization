...
conditions = [name for name in os.listdir(f"{log_dir}/{scenarios[0]}") if os.path.isdir(os.path.join(f"{log_dir}/{scenarios[0]}", name)) and name.endswith("generalization")]
...
for combination in itertools.product(scenarios, conditions, IDs, epochs):
    ...
    params = [simulation for simulation in ID_to_param_file if (simulation['simulation_condition']==simulation_condition.replace("-generalization", "") and simulation['ID']==ID)][0]
...
data_df0.to_csv(f'{log_dir}/simulated_results_generalization_format0.csv', index=False)
data_df1.to_csv(f'{log_dir}/simulated_results_generalization_format1.csv', index=False)