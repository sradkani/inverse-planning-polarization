...
job_ID = sys.argv[4]   
epoch = sys.argv[5]  
prev_action = sys.argv[6]

log_dir = f"{directory}/{scenario}/{simulation_condition}/ID_{job_ID}/epoch_{epoch}/config"
posterior_dir = f"{directory}/{scenario}/{simulation_condition}/ID_{job_ID}/epoch_{int(epoch)-1}/posteriors/{prev_action}"
file_names = [filename for filename in os.listdir(posterior_dir) if not filename.startswith("expected_")]
...
param_df['simulation'] = simulation_condition
