  ...
  do
    ...
    do
      ...
      if [ "$epoch" -eq 1 ]; then
        ...
        prev_epoch=$((epoch - 1))
        prev_action=$(jq -r --arg EPOCH "$prev_epoch" '.[] | select(.epoch == $EPOCH) .action' general_config/actions.json)
      ...
      action=$(jq -r --arg EPOCH "$epoch" '.[] | select(.epoch == $EPOCH) .action' general_config/actions.json)
